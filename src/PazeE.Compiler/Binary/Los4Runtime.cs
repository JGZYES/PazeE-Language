...
/// <summary>LeonOS 4 静态运行时：通过 int 0x80 系统调用提供标准 C 库函数实现。
/// 所有函数以 System V AMD64 ABI 调用，注入到 .text 段末尾，由 ElfWriterLos4 解析外部符号引用。
/// LeonOS 4 系统调用约定与 Linux x86_64 一致（rax=调用号, rdi/rsi/rdx...=参数），用 int 0x80 而非 syscall。</summary>
public static class Los4Runtime
{
    ...
    public static (byte[] code, Dictionary<string, int> offsets, List<(int off, string content)> rdataRefs, List<(int off, string name, int bssOff)> bssRefs, int bssSize) Generate()
    {
        var b = new Rt();
        // 预分配 GUI 持久状态（BSS 全局）：gui_fd, gui_win_id
        b.AllocBss("gui_fd", 8);
        b.AllocBss("gui_win_id", 8);
        ...
        // GUI 函数
        Reg("gui_init", GenGuiInit);
        Reg("gui_window_create", GenGuiWindowCreate);
        Reg("gui_window_text", GenGuiWindowText);
        Reg("gui_window_present", GenGuiWindowPresent);
        Reg("gui_event_poll", GenGuiEventPoll);
        Reg("gui_event_wait", GenGuiEventWait);
        Reg("gui_window_destroy", GenGuiWindowDestroy);
        Reg("gui_cleanup", GenGuiCleanup);
        return (b.Finish(), o, b.RdataRefs, b.BssRefs, b.BssSize);
    }
    ...
    private static void GenTimeGetUtcRaw(Rt b)
    {
        ...
        b.MovEax(SYS_open); b.Int80();             // rax = fd
    ...
    // ============ GUI ============
    // LeonOS 4 GUI：open("/dev/leonos_gui") + ioctl 实现。gui_fd/gui_win_id 存于 BSS 全局。
    // 结构体布局（来自 leonos/gui.h）：
    ...
    //                          u8 buttons, keycode, pressed, reserved; }           = 40 字节
    //   leonos_gui_wait_app_event { event[40]; u32 timeout_ms; }                   = 44 字节

    // int gui_init(void) — 打开 GUI 设备，fd 存入 gui_fd。
    private static void GenGuiInit(Rt b)
    {
        int retOk = b.Lbl(), retFail = b.Lbl();
        b.PushRbp(); b.MovRbpRsp(); b.SubRsp(16);
        // if (gui_fd != 0) return 0;
        b.MovImm64BssRef(0, "gui_fd");          // rax = &gui_fd
        b.LoadReg32(0, 0);                       // eax = gui_fd
        b.Test(0, 0);
        b.Jne(retOk);
        // fd = open("0:/dev/leonos_gui", O_RDWR=2, 0)
        b.MovImm64RdataRef(7, Los4GuiDevice);    // rdi = device path
    ...
    {
        // 参数：rdi=title(7), rsi=width(6), rdx=height(2)
        // 栈布局：[rbp-8]=title, [rbp-16]=width, [rbp-24]=height, [rbp-56..-25]=gui_create(32 字节)
        int retFail = b.Lbl();
        b.PushRbp(); b.MovRbpRsp(); b.SubRsp(64);
        b.StoreRbp(-8, 7);                       // 保存 title
        b.StoreRbp(-16, 6);                      // 保存 width
        b.StoreRbp(-24, 2);                      // 保存 height
        // gui_init()（内部已处理重复调用）
        b.CallName("gui_init");
        b.Test(0, 0);
        b.Js(retFail);                           // gui_init < 0 → -1
        // 构造 leonos_gui_create @ [rbp-56]，用 rdi 作游标逐字段填充
        b.LeaRbp(7, -56);                        // rdi = &gc
        b.LoadRbp(0, -16); b.StoreReg32(7, 0);   // gc.width  = width  ([rdi+0])
        b.AddImm(7, 4);
        b.LoadRbp(0, -24); b.StoreReg32(7, 0);   // gc.height = height ([rdi+4])
        b.AddImm(7, 4);
        b.LoadRbp(0, -8);  b.StoreReg64(7, 0);   // gc.title  = title  ([rdi+8])
        b.AddImm(7, 8);
        b.Xor(0, 0);       b.StoreReg64(7, 0);   // gc.text   = NULL   ([rdi+16])
        b.AddImm(7, 8);
        b.Xor(0, 0);       b.StoreReg32(7, 0);   // gc.flags  = 0      ([rdi+24])
        // ioctl(gui_fd, CREATE_WINDOW, &gc)
        b.MovImm64BssRef(7, "gui_fd"); b.LoadReg32(7, 7);  // rdi = gui_fd
        b.MovImm64(6, LEONOS_GUI_IOCTL_CREATE_WINDOW);      // rsi = request
        b.LeaRbp(2, -56);                                   // rdx = &gc
        b.MovEax(SYS_ioctl); b.Int80();                     // rax = window_id
        b.Test(0, 0);
        b.Js(retFail);                           // < 0 → -1
        // gui_win_id = rax
        b.Mov(1, 0);                             // rcx = id（保留）
        b.MovImm64BssRef(0, "gui_win_id");
        b.StoreReg32(0, 1);                      // gui_win_id = id
        b.Xor(0, 0);                             // return 0
        b.Leave(); b.Ret();
        b.Mark(retFail);
        b.MovImm64(0, -1);                       // return -1
    ...
    {
        // 参数：rdi=ev(7)
        // 结构 leonos_gui_app_event @ [rbp-48]（40 字节）
        int noEvent = b.Lbl();
        ...
        b.MovImm64BssRef(7, "gui_fd"); b.LoadReg32(7, 7);
        b.MovImm64(6, LEONOS_GUI_IOCTL_WINDOW_EVENT);
        b.LeaRbp(2, -48);
        b.MovEax(SYS_ioctl); b.Int80();          // rax = 0(无事件) / 1(有事件)
        b.Test(0, 0);
    ...
    private static void GenGuiEventWait(Rt b)
    {
        ...
        // 结构 leonos_gui_wait_app_event @ [rbp-56]（44 字节）：event[40] + timeout_ms[4]
        ...
        // timeout_ms @ [rbp-56+40] = [rbp-16]
        b.LoadRbp(0, -72);
        b.LeaRbp(1, -16); b.StoreReg32(1, 0);
        // ioctl(gui_fd, WAIT_WINDOW_EVENT, &wait)
        b.MovImm64BssRef(7, "gui_fd"); b.LoadReg32(7, 7);
        b.MovImm64(6, LEONOS_GUI_IOCTL_WAIT_WINDOW_EVENT);
        b.LeaRbp(2, -56);
        b.MovEax(SYS_ioctl); b.Int80();
    ...
    {
        // 参数：rdi=win(7)
        // ioctl(gui_fd, DESTROY_WINDOW, &win_id)；win_id 存于 [rbp-8]
        b.PushRbp(); b.MovRbpRsp(); b.SubRsp(16);
        b.StoreRbp(-8, 7);                       // [rbp-8] = win（StoreRbp: mov [rbp+disp], r64）
        b.MovImm64BssRef(7, "gui_fd"); b.LoadReg32(7, 7);  // rdi = gui_fd
        b.MovImm64(6, LEONOS_GUI_IOCTL_DESTROY_WINDOW);    // rsi = request
        b.LeaRbp(2, -8);                                    // rdx = &win_id
        b.MovEax(SYS_ioctl); b.Int80();
        b.Xor(0, 0);                             // return 0
        b.Leave(); b.Ret();
    }

    // int gui_cleanup(void) — 关闭 GUI 设备。
    private static void GenGuiCleanup(Rt b)
    {
        ...
        b.Test(7, 7);
        int skip = b.Lbl();
        b.Je(skip);                                         // gui_fd == 0 → skip
        b.MovEax(SYS_close); b.Int80();
        // gui_fd = 0
        b.MovImm64BssRef(7, "gui_fd"); b.Xor(0, 0); b.StoreReg32(7, 0);
        b.Mark(skip);
        b.Xor(0, 0);                             // return 0
        b.Leave(); b.Ret();
    }
    ...
    private static void GenExit(Rt b) { b.MovEax(SYS_exit); b.Int80(); b.Ret(); }
    ...
    private static void GenMalloc(Rt b)
    {
        ...
        b.Mov(6, 7);                        // rsi = length
        b.XorEdiEdi();                      // rdi = addr = 0
        b.MovEdx(PROT_READ | PROT_WRITE);   // rdx = prot = 3
        b.MovImm64(10, MAP_PRIVATE | MAP_ANONYMOUS); // r10 = flags = 0x22
        b.MovImm64(8, -1);                  // r8 = fd = -1
        b.Xor(9, 9);                        // r9 = offset = 0
        b.MovEax(SYS_mmap); b.Int80();      // rax = ptr
        b.PopRbp(); b.Ret();
    }

    private static void GenFree(Rt b) { b.Ret(); } // mmap 内存由内核管理，free 为空操作
    ...
    private static void GenCalloc(Rt b)
    {
        ...
        b.Mov(6, 7);                        // rsi = length = total
        b.XorEdiEdi();                      // rdi = addr = 0
        b.MovEdx(PROT_READ | PROT_WRITE);   // rdx = prot
        b.MovImm64(10, MAP_PRIVATE | MAP_ANONYMOUS); // r10 = flags
        b.MovImm64(8, -1);                  // r8 = fd = -1
        b.Xor(9, 9);                        // r9 = offset = 0
        b.MovEax(SYS_mmap); b.Int80();      // rax = ptr
        // memset(ptr, 0, total)
    ...
    private static void GenFputs(Rt b)
    {
        ...
        b.Mark(done);
        b.Mov(2, 1); b.PopR(6);
        b.MovEdi(1); b.MovEax(SYS_write); b.Int80();
        b.Xor(0, 0);
        b.PopRbp(); b.Ret();
    }
...
internal sealed class Rt
{
    ...
    internal void Int80() { _code.Add(0xCD); _code.Add(0x80); }
